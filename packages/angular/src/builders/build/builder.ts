  ...
  const normalized = await normalizeFederationOptions(
    {
      ...
      federationConfig: inferConfigPath(federationTsConfig, context.workspaceRoot, nfBuilderOptions.federationConfigFilePath),
...
function inferConfigPath(tsConfig: string, workspaceRoot: string, federationConfigFilePath = 'federation.config.mjs'): string {
  const relProjectPath = path.dirname(tsConfig);

  const mjsRelPath = path.join(relProjectPath, federationConfigFilePath);
